pro frames_to_positions, , pos_info
...
for i=0,foo-1 do begin
  ; Select the period of time for each different MP observations
	test=where(hv.sc_time_first_hack lt hv.position_time[i*2+1] and hv.sc_time_last_hack gt hv.position_time[i*2],ntest)  
	; There is the chance that I can get too many files here if there are multiple acquisitions at a given scan mirror position.  
	; You have to cut them up later by using the tab_byte_numbers to figure out if more than one acquisition has occured. 
	
	if (ntest gt 0) then begin
	  ; Get the number of frame
		num_frames[i]=size(test,/n_elements)

		; Assign the frame list
		frame_list[i,0:num_frames[i]-1]=test
		pos_list[i]=hv.position_num[i*2]
		; Compute the time interval / start time / end time
		...
		if (ntest gt 2) then begin
		    ; Writes out, for each frame, the position of the MP
        position_per_frame[test[1:num_frames[i]-2]]=pos_list[i]
		endif
		
		; If the MP of the first frame of that list is not zero, write the MP of the current sequence. Otherwise, uses -1 instead.
		...
		; If the MP of the last frame of that list is not zero, write the MP of the current sequence. Otherwise, uses -1 instead.
