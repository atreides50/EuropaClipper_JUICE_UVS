pro readjunofitsfast_ver6_special,filename,output,maps=maps,starthackpointer=starthackpointer,deadtime=deadtime,analog=analog,plots=plots
...
;  remove_frame, FRAME_COUNTER,SC_RCVD_TIME,FIRST_TIMEHACK,LAST_TIMEHACK,SC_TIME_FIRST_HACK,SC_TIME_LAST_HACK,FRAME_QUALITY,FRAME_TAG,FINAL_FRAME,QUALITY_FLAG,MEMORY_SIDE,HACK_RATE,NUM_TIMEHACKS,FRMSUM,CHKSUM,TYPE,CLOCK_PERIOD,NUM_ACQS,EXPECTED
  ...
  if (keyword_set(deadtime)) then begin
	  ...
	  dig_point_starthack=fltarr(s[0])
;	  dig_point_starthack2=fltarr(s[0])
	  s2=size(dig_hack_time,/dim)
  if (keyword_set(starthackpointer)) then begin
	  ;the current version of lima leaves out the first hack from every frame within the dig_hack_time array.  This will probably change in the future.  If that is the case then I will have to shift by 1*2^hack_rate[i] only at the begining of each acquisition insead of every frame
;	  t1=systime(1)
...
;		  dig_point_starthack[i]=dig_point_starthack[i-1]+where(first_timehack[i]+1*2^hack_rate[i] eq dig_hack_time[dig_point_starthack[i-1]:min([long(dig_point_starthack[i-1]+32768),s2[0]-1])])
		...
		if (~nfoo) then foo=where(first_timehack[i]+1*2^hack_rate[i] eq dig_hack_time[dig_point_starthack[i-1]:min([long(dig_point_starthack[i-1]+32768),s2[0]-1])],nfoo)
