...
PRO XV_SET_DEFAULTS
   ...
   PRINT, "Setting default MASTER for earth camera to: ", d_master_e
   ...
   PRINT, "Setting default MASTER for visible light camera to: ", d_master_l
...
;----------------------------------------------------------
PRO XV_VPOINT, SC_POS, LOOK, ALT, NP, POS1, POS2
						;; alt = altitude
   ...
   aq =  (x2-x1)^2 / AxA + $
    ...
    XV_QUAD, AQ, BQ, CQ, NP, ROOT1, ROOT2
...
;-
;-------------------------------------------------------------
FUNCTION XV_RUNLENGTH,X,R, help=hlp
...
;-
;-------------------------------------------------------------
function XV_intersect,x,y, help=hlp
   ...
   xs = XV_runlength(x(sort(x)))   ; Keep only unique elements.
   ys = XV_runlength(y(sort(y)))
...
PRO XV_ALT_SZA
   ...
   XV_VPOINT, sc_pos, lookv_gci, assumedalt, np, loc1, loc2
...
;
;----------------------------------------------------------
FUNCTION XV_PROCESS_RECORDS, DISPLAY = display, TSTART = tstart, TSTOP = tend, E0 = e0, E1 = e1
   ...
   IF sensor eq 0 THEN PRINT, "Processing Data for Earth Camera."
   IF sensor eq 1 THEN PRINT, "Processing Data for Low Resolution Camera."
   ...
   IF (fl_h_smooth_ec4) THEN BEGIN
       ...
       PRINT, "Data file processing to include: Horizontal Smooth EC4."

   ENDIF

   IF (fl_h_smooth_ec6) THEN BEGIN

       PRINT, "Data file processing to include: Horizontal Smooth EC6."

   ENDIF

   IF (fl_sub_cosmic_ray) THEN BEGIN

       PRINT, "Data file processing to include: Cosmic Ray Subtraction."

   ENDIF

   IF (fl_sub_slopes)     THEN BEGIN

       PRINT, "Data file processing to include: Slope Subtraction."

   ENDIF

   IF (fl_remove_weave)   THEN BEGIN

       PRINT, "Data file processing to include: Remove Weave."

   ENDIF

   IF (fl_flat_field)     THEN BEGIN

       PRINT, "Data file processing to include: Flat Field."

   ENDIF

   IF (fl_dayglow_sub)    THEN BEGIN

       PRINT, "Data file processing to include: Dayglow Subtraction."

   ENDIF

   IF (fl_nightglow_min)  THEN BEGIN

       PRINT, "Data file processing to include: Nightglow Minimum."

   ENDIF

   IF (fl_smooth_filter)  THEN BEGIN

       PRINT, "Data file processing to include: Smooth Filter."
   ...
   PRINT, FORMAT = '("Processed: ", I0, " records.")', rec_count
...
; (this is done here because XV_CLimits needs some Improparams)
      ...
      IF fc eq 0 THEN XV_SCALE_COLOR_TABLE, Limits [0], Limits [1]
...
PRO XV_REMOVE_VV, cdf
   ...
   FOR i = 0, N_TAGS (skeleton_buf) - 1 DO BEGIN
      ...
      IF  is_virtual THEN BEGIN   
          ...
          PRINT, "Deleting ", v.name, " as variable is of type 'virtual'."
   ...
   PRINT, FORMAT = '("Creating CDF with ", I0, " variables.")', N_TAGS (skeleton_buf)
...
;   END
   ...
   ; xv_check_input, source, target, arch, af, master, log, fc

   ; Create a display window
   IF KEYWORD_SET (display) THEN WINDOW, 0, XSIZE=512, YSIZE=256
   ...
   FOREACH cdf, list DO BEGIN 
       ...
       WHILE (t0 lt ti [1]) DO BEGIN
          ...
          rec = xv_process_records (DISPLAY = display, TSTART = tmp, TSTOP = tstop, E0 = t0, E1 = t1)
       ...
       IF  proc_rec ne t_cdf_recs THEN BEGIN
           ...
           PRINT, "ERROR: ############################################"
           PRINT, FORMAT = '("Source CDF had ", I0, " records.")', t_cdf_recs
           PRINT, FORMAT = '("Only ", I0, " were actually processed.")', proc_rec
