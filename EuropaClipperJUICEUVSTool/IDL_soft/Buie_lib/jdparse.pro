...
function jdparse,in_str,DATETMPL=in_datetmpl,STRICT=strict,ERRFLG=errflg, $
   ...
   if fulldate gt 0 then begin
      mon2num,in_str,instr,fulldate=fulldate
   endif else begin
      instr=in_str
   endelse
               ...
               'D' : begin
                  if val ge 1 and val le 31 then begin
                     ...
                     if not quiet then begin
                        ...
                        print,'JDPARSE: [',instr[i],']  tok=[',strjoin(toktmpl),']'
                     endif
                     errflg[i]=1B
                  endelse
               end
