...
;-
pro puttran,inst,date,filter,color1,color2,tran,transig,jdref, $
    refcolor,refam,nobs,replaced,fitted,chi2,quality, $
   ...
   if badpar(tran,[4,5],         1,caller=self+'(tran) ',DIMEN=d) then return
   if ( d ne 7 ) then begin
      print, self + '(tran) ', 'must have 7 elements, found:', d
      return
   endif
   if badpar(transig,[4,5],      1,caller=self+'(transig) ',DIMEN=d) then return
   if ( d ne 7 ) then begin
      print, self + '(transig) ', 'must have 7 elements, found:', d
      return
   endif
   if badpar(jdref,5,            0,caller=self+'(jdref) ') then return
   if badpar(refcolor,[4,5],     0,caller=self+'(refcolor) ') then return
   if badpar(refam,[4,5],      0,caller=self+'(refam) ') then return
   if badpar(nobs,[1,2,3],       0,caller=self+'(nobs) ') then return
   if badpar(fitted,[0,1,2,3],   1,caller=self+'(fitted) ', DIMEN=d) then return
   if ( size(fitted,/type) ne 0 and d ne 7 ) then begin
      print, self + '(fitted) ', 'must have 7 elements, found:', d
      return
   endif
   ...
   gettran,inst,date,filter,color1,color2,tr,trs,jdr,colr,amr,found, $
            oldfitted,oldnobs,oldchi2,oldquality, $
            DATABASE=database,TABLENAME=tablename,/SILENT,VERBOSE=verbose
   if found then begin
   ...
   ; color reference
   thevalues = thevalues+commaspc+string(refcolor,FORMAT='(f8.5)')

   ; airmass reference
   thevalues = thevalues+commaspc+string(refam,FORMAT='(f8.5)')
