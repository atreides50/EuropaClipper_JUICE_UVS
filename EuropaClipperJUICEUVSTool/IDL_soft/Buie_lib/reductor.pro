...
;  instruments 
   ...
   mntpts_PCCD=['/net/suhuy/raid2']
;   mntpts_Nasacam =  ['/media/usbdisk']
   mntpts_Nasacam =  ['/net/fiona/raid1','/net/suhuy/raid2']
   mntpts_site2k =  ['/net/suhuy/raid2']
...
;   lon = (111.0+32.0/60.0+10.601/3600.0)/180.0*!pi
   ...
   if not exists(infofile) then begin
      ...
      if inst eq 'Nasacam' then begin
         default_oplines = [ $
            'tr B B 1 1 2 ok', $
            'tr V V 2 1 2 k2 0. 0. s2 0. 0. a2 0. 0. ok', $ 
            'tr V V 2 2 3 k2 0. 0. s2 0. 0. a2 0. 0. ok', $
            'tr R R 3 2 3 k2 0. 0. s2 0. 0. a2 0. 0. ok', $
            'sl B V B V 1 2 ok', $
            'sl V R V R 2 3 ok'  $ 
...
;            'dp 9 M BV p9 0 0.0   SAO_160288 k2 0. 0. ct 0. 0. ok' ]
         ...
         default_oplines = [ $
            'tr 1 B 1 1 2 ok', $
            'tr 2 V 2 1 2 k2 0. 0. s2 0. 0. a2 0. 0. ok', $ 
            'tr 2 V 2 2 3 k2 0. 0. s2 0. 0. a2 0. 0. ok', $
            'tr 3 R 3 2 3 k2 0. 0. s2 0. 0. a2 0. 0. ok', $
            'sl 1 2 B V 1 2 ok', $
            'sl 2 3 V R 2 3 ok'  $ 
;            '2c a???? 0 2 V 2 2 3 3 R 3 2 3 ok', $
;            'lc a???? 0 3 R 3 2 3 0.4? 0.0? ok', $
...
;      if inst eq 'Nasacam' then target = '20' + data
   ...
   for i=0,numop-1 do begin
      ...
      parseop,info.oplist[i],op
      if op.error then continue
      if op.ok and not force then continue
...
; ------------------------------------------------------------------------------
         ...
         'tr': begin
            ...
            zfil = where(pdata.fil eq op.vals[0],count)
            if count eq 0 then begin
               print,op.str
               print,'No data found in filter ',op.vals[0]
               goto,skipit
            endif

            if op.k2fit then k2=[999.0,0.] else k2=op.k2
            if op.ctfit then ct=[999.0,0.] else ct=op.ct
            if op.kfit  then  k=[999.0,0.] else  k=op.k
            if op.c2fit then c2=[999.0,0.] else c2=op.c2
            if op.x2fit then x2=[999.0,0.] else x2=op.x2

            if saveallplots then begin
               psname = 'tr.'+op.vals[2]+','+op.vals[3]+'-'+op.vals[4]+'.ps'
            ...
            if not nosave then begin
               ...
               spawn, '/usr/bin/whoami', whois
               openw, loglun,infologfile, /GET_LUN,/APPEND
               printf,loglun,tis+' '+whois+' '+op.str+' '+psname
               free_lun, loglun
            endif
            transf,pdata.stand[zfil],pdata.jd[zfil], $
               fix(op.vals[2]),fix(op.vals[3]),fix(op.vals[4]), $
               pdata.am[zfil],pdata.mag[zfil],pdata.err[zfil],tmpbad, $
               title=info.inst+' '+info.data,other=pdata.time[zfil], $
               olab='UT Time',k2=k2,cterm=ct,ktime=op.kt,NOEDIT=noedit, $
               /chi,tagdate=info.data,taginst=info.inst,magresid=magresid, $
               NOSAVE=nosave,NODB=nosave,TABLENAME=transftable, $
               DVERBOSE=dverbose,amsq=x2,colsq=c2
            ...
            stdcheck,landcat,pdata.stand[zfil], $
               [ [ replicate(fix(op.vals[2]),count) ], $
                  [ replicate(fix(op.vals[3]),count) ], $
                  [ replicate(fix(op.vals[4]),count) ]    ], $
               stdflg
            z=where(stdflg eq 0, count0)
            if count0 ne 0 then begin
               tmpnam=pdata.stand[zfil[z]]
               tmpnam=tmpnam[uniq(tmpnam,sort(tmpnam))]
               print,' Objects to reduce for ',landfil[fix(op.vals[2])], $
                  ' wrt (',landfil[fix(op.vals[3])],'-',landfil[fix(op.vals[4])],')'
...
; ------------------------------------------------------------------------------
         ...
         'lc': begin
            ...
            colorname =''
            if op.vals[5] eq '1' and op.vals[6] eq '2' then colorname = 'B-V'
            if op.vals[5] eq '2' and op.vals[6] eq '3' then colorname = 'V-R'
            
            ; First, must fetch the transformation coefficients for this obsn
            gettran,info.inst,info.data,fix(op.vals[4]),fix(op.vals[5]),fix(op.vals[6]), $
               tran,transig,jdref,refcolor,refam, $
            ...
            lnam=strlowcase(op.vals[0])
            dnam=info.data+'.'+lnam
            oldbad=pdata.bad
            if saveallplots then begin
               psname = 'lc.'+op.vals[0]+':'+op.vals[1]+'_'+op.vals[4]+','+op.vals[5]+'-'+op.vals[6]+'.ps'
               device,filename=psname
            endif else begin
               psname=''
            endelse
            ...
            ltcrv2,pdata.stand,pdata.fil,pdata.jd,pdata.time,pdata.am, $
                  ...
                  float(op.vals[7]),float(op.vals[8]),op.vals[0],fix(op.vals[1]),op.vals[2], $
                  tran,transig,jdref,refcolor,refam, $
                  filtname=op.vals[3],file=dnam,badflags=tmpbad,NODB=nosave, $
            ...
            z=where(pdata.stand eq op.vals[0] and pdata.serial eq fix(op.vals[1]) and $
                    pdata.fil eq op.vals[2],count0)
            if count0 ne 0 then begin
               pdata.used[z] = 1
...
; ------------------------------------------------------------------------------
         ...
         '2c': begin
            ...
            transig[3]=0. ; turn off zeropoint error
            gettran,info.inst,info.data,fix(op.vals[9]),fix(op.vals[10]),fix(op.vals[11]), $
               tran1,transig1,jdref1,refcolor1,refam1, $
               /db,VERBOSE=dverbose
            sz=size(tran1)
            if sz[0] eq 0 then begin
               errstr='No transformation record was found.  Aborting.'
            ...
            oldbad=pdata.bad
            if saveallplots then begin
               psname = '2c.'+op.vals[0]+':'+op.vals[1]+'_'+op.vals[4]+','+op.vals[9]+'.ps'
               device,filename=psname
            endif else begin
               psname=''
            endelse
            ...
            ltcrv2c,pdata.stand,pdata.fil,pdata.jd,pdata.time,pdata.am, $
                  pdata.serial,pdata.mag,pdata.err, $
                  op.vals[0],fix(op.vals[1]),[op.vals[2],op.vals[7]],tran,transig,jdref, $
                  refcolor,refam, $
                  color,colorsig,badflags=tmpbad, $
                  filtname=[op.vals[3],op.vals[8]],error=error,file=[dnam,dnam], $
            ...
            if not nosave then begin
               ...
               colorfile=lnam+'.'+op.vals[3]+'-'+op.vals[8]
               ...
               meanjd = mean(jdcolor)
               colornm = op.vals[3] + '-' + op.vals[8]
               print, 'updating ',  datatable, ' for ', op.vals[0], $
               ...
               dbphot, refid, op.vals[0], meanjd, colornm, color, colorsig, $
                    TABLE=datatable,/CLEANBYOBJFIL,NREMOV=nr,VERBOSE=dverbose
               print, nr, ' previous observations for ', op.vals[0],  $
            ...
            ; flag anything that was used
            z=where(pdata.stand eq op.vals[0] and pdata.serial eq fix(op.vals[1]) and $
                    (pdata.fil eq op.vals[2] or pdata.fil eq op.vals[7]),count0)
            if count0 ne 0 then begin
               pdata.used[z] = 1
...
; ------------------------------------------------------------------------------
         ...
         'sl': begin
            ...
            if fil1 eq 2 and fil2 eq 3 then begin
               ...
               print,op.str
               print,self,'Error!  Unsupported color for sl operation'
               goto,skipit
            endelse

            ; First, must fetch the transformation coefficients for this obsn
            gettran,info.inst,info.data,fa,c1,c2,tran1,tran1sig,jdref1, $
                  refcolor1,refam1,TABLENAME=transftable,VERBOSE=dverbose
            gettran,info.inst,info.data,fb,c1,c2,tran2,tran2sig,jdref2, $
                  refcolor2,refam2,TABLENAME=transftable,VERBOSE=dverbose
            sz=size(tran1sig)
            if sz[0] eq 0 then begin
               errstr='No transformation record was found.  Aborting.'
               print,errstr
            ...
            zfil = where( (pdata.fil eq op.vals[0] or $
                           pdata.fil eq op.vals[1]) and $
                          strmid(pdata.stand,0,1) ne 'a' and $
                          strmid(pdata.stand,0,1) ne 'c' and $
                          strmid(pdata.stand,0,1) ne 'p' ,count)
            if count eq 0 then begin
               print,op.str
               print,'No data found in filter ',op.vals[0],' and ',op.vals[1]
               goto,skipit
            endif
            ...
            if countdo eq 0 then begin
               print,op.str
               print,'No non-standard objects to reduce in ', $
                  op.vals[0],' and ',op.vals[1]
               goto,skipit
            endif
            zfil=zfil[z]
         ...
         'dp': begin
            ...
            case op.vals[1] OF
               ...
               'M': fa=9
                  ...
                  print,op.str
                  print,'Unrecognized filter [',op.vals[1],']'
                  goto,skipit
               endelse
            endcase

            c1=strmid(op.vals[2],0,1)
            colorname = c1 + '-'
            case c1 OF
               'B': c1=1
               'V': c1=2
               'R': c1=3
                  ...
                  print,op.str
                  print,'Unrecognized filter in color [',op.vals[2],']'
                  goto,skipit
               endelse
            endcase

            c2=strmid(op.vals[2],1,1)
            colorname += c2
            case c2 OF
               'B': c2=1
               'V': c2=2
            ...
            loadtrans=1
            transdate=info.data
            if op.td ne '' then transdate=op.td

            if op.k2fit then begin
               loadtrans=0
               k2=op.k2
               if not op.ctfit then ct=[0.,0.]
               if not op.c2fit then c2=[0.,0.]
               if not op.x2fit then x2=[0.,0.]
            endif
            if op.ctfit then begin
               loadtrans=0
               ct=op.ct
               if not op.k2fit then k2=[0.,0.]
               if not op.c2fit then c2=[0.,0.]
               if not op.x2fit then x2=[0.,0.]
            endif
            if op.kfit then begin
               k=op.k
            endif
            if op.c2fit then begin
               loadtrans=0
               c2=op.c2
               if not op.k2fit then k2=[0.,0.]
               if not op.ctfit then ct=[0.,0.]
...
;            k =[999.,0.]
            ...
            if fa ne 9 and loadtrans then begin
               gettran,info.inst,op.td,fa,c1,c2,tran,transig,jdref, $
                     refcolor,refam,/DB,TABLENAME=transftable,VERBOSE=dverbose
               sz=size(tran)
               if sz[0] eq 0 then begin
                  print,op.str
                  print,'No transformation found for '+op.td+', '+ $
                         op.vals[1]+' - '+op.vals[2]
                  goto,skipit
               endif
               k2 = [tran[1],transig[1]]
            ...
            if fa ne 9 then begin
               rdphocat,'/net/frakir/raid/buie/photometry/buiestd.dat', $
                        lname,lmags,lcod
               cleanname = repchar(op.vals[6],'_',' ')
            ...
            stdname = repchar(op.vals[6],'_',' ')
            op.vals[3] = repchar(op.vals[3],'_',' ')
            objname = naifname(op.vals[3])
            dnam=info.data+'_'+op.vals[1]+'.'+objname

            if not forcek then k = [999.,0.]

            if saveallplots then begin
               psname = 'dp.'+op.vals[3]+':'+op.vals[4]+ $
                        '_'+op.vals[1]+','+op.vals[2]+'.ps'
               device,filename=psname
            endif else begin
               psname=''
            endelse
            ...
            ltcrv,pdata.stand,pdata.fil,pdata.jd,pdata.am,pdata.serial, $
               pdata.mag,pdata.err,op.vals[3],fix(op.vals[4]),stdname, $
               op.vals[0],jdout,K2=k2,CTERM=ct,AMSQ=x2,COLSQ=c2, $
               OCOLOR=[float(op.vals[5]),0.],SCOLOR=[scolor,0.], $
               refcolor=refcolor,refam=refam, $
               STDMAG=[smag,0.],FILTNAME=op.vals[1],OBJNAME=objname, $
               FILE=dnam,BAD=tmpbad,FORCE=k,NODB=nosave, $
               NOSAVE=nosave,REFID=refid,TABLE=datatable,PLOTWIN=4, $
            ...
            if objname eq 'Pluto' and not nosave then begin
               ...
               tag=string(info.data,op.vals[1]+blanks,format='(a,1x,a2)')
               outinfo=string(info.inst+blanks,op.vals[6]+blanks,smag,scolor, $
                                 format='(1x,a10,1x,a16,1x,f7.4,1x,f6.4)')
               print,tag+outinfo
               repwrite,histname,tag,tag+outinfo
            endif
            ; flag anything that was used
            z=where( ((pdata.stand eq op.vals[3] and $
                       pdata.serial eq fix(op.vals[4])) or $
                      (pdata.stand eq stdname)) and $
                       pdata.fil eq op.vals[0],count0)
            if count0 ne 0 then begin
               pdata.used[z] = 1
               print,'dp rule, marking ',strn(count0),' as used.  ', $
                     'Total now ',strn(long(total(pdata.used))),'/', $
                     strn(pdata.nobs),' used (', $
                     strtrim(string(float(total(pdata.used))/ $
                        float(pdata.nobs)*100.0,format='(f10.1)'),2),'%)'
            endif
            if saveallplots then begin
