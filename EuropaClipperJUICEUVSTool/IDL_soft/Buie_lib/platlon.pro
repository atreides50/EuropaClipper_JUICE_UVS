...
;  EPH   - Optionally, you can provide the ephemeris information.  The default
;            is to compute geocentric circumstants at the input time.  This
;            information should be a vector as would be returned from a
;            code=22 request: [ra,dec,xobj,yobj,zobj,xsun,ysun,zsun].  The
;            positions should all be in units of AU.  The ra,dec should be
;            in radians.  The positions are relative to the observer.
; OUTPUTS:
...
;  2015/05/25 - MWB, added EPH input keyword.
;-
pro platlon,jd,elat,elon,slat,slon,polang,phang,earth,sun,elong, $
       ORBIT=in_orbit,EPH=in_eph
   ...
   if badpar(in_eph,[0,4,5],1,caller=self+'(EPH) ',type=ephtype) then return
