...
pro transf,names,jd,filter,color1,color2,airm,mag,err,bad, $
      ...
      NOEDIT=noedit, OTHER=other,OLAB=olab,TITLE=title,FILES=files, $
      CHISCALE=chiscale,TAGDATE=in_tagdate,TAGINST=in_taginst, $
      HISTFILE=histfile,MAGRESID=magresid,NODB=nodb,DATABASE=database, $
      TABLENAME=tablename,NOSAVE=nosave,DVERBOSE=dverbose, $
      COLSQ=colsq,AMSQ=amsq,REFCOLOR=refcolor,REFAM=refam
   ...
   if badpar(colsq,[0,4,5],1,caller=self+'[COLSQ] ',default=999.0) then return
   if badpar(amsq,[0,4,5],1,caller=self+'[AMSQ] ',default=999.0) then return
   ...
   if badpar(refcolor,[0,4,5],0,   caller=self+'(REFCOLOR) ',default=999.0) then return
   if badpar(refam,[0,4,5],0,   caller=self+'(REFAM) ',default=999.0) then return
   ...
   if refcolor gt 900.0 then autoref=1 else autoref=0
   if refam    gt 900.0 then autoam =1 else autoam =0
   ...
   if amsq[0] ne 999.0 then begin
      trans[5]   = amsq[0]
      transig[5] = amsq[1]
      fit[5]     = 0
   endif

   if colsq[0] ne 999.0 then begin
      trans[6]   = colsq[0]
      transig[6] = colsq[1]
      fit[6]     = 0
   endif
   ...
   after_edit:
      ...
      if autoref then refcolor = mean(std_col[zgood])
      if autoam  then refam    = mean(minmax(oairm[zgood]))

      ; Fill independent variable vectors
      dair = oairm-refam
      dcol = std_col-refcolor
      x[0,*] = -dair
      x[1,*] = -dcol*dair
      x[2,*] = dcol
      x[3,*] = 1.0
      ...
      ; Setup the depedent vector and uncertainty
      y = std_mag - omag
      ysig = oerr^2
      ...
      ysig = sqrt(ysig)
;print,'Reference color is   ',refcolor
;print,'Reference airmass is ',refam
;for ii=0,goodcount-1 do begin
;   print,ii,y[zgood[ii]],ysig[zgood[ii]],oairm[zgood[ii]],std_col[zgood[ii]], $
;            dair[zgood[ii]],dcol[zgood[ii]], $
;      format='(i3,1x,f7.4,1x,f6.4,1x,f5.3,1x,f6.3,1x,f6.3,1x,f6.3)'
;endfor
      ...
      if not first or not chiscale or edit then begin
         ...
         print,'airmass squared         ',trans[5],pm,transig[5],transtr[5],format=fmt
         print,'color squared           ',trans[6],pm,transig[6],transtr[6],format=fmt
         print,'Reference color         ',refcolor
         print,'Reference airmass       ',refam
         if first then $
         ...
         if saveit and ( not first or not edit ) then begin
            ...
            printf,lusum,'airmass squared         ',trans[5],pm,transig[5],transtr[5],format=fmt
            printf,lusum,'color squared           ',trans[6],pm,transig[6],transtr[6],format=fmt
            printf,lusum,'Reference color         ',refcolor
            printf,lusum,'Reference airmass       ',refam
            if first then $
         ...
         if plotit then begin
            ...
            ploterror,-x[0,zgood]+refam,y_var1[zgood],oerr[zgood],psym=8, $
            ...
            ploterror,x[2,zgood]+refcolor,y_var3[zgood],oerr[zgood],psym=8, $
   ...
   ;            charsize=cs,xtit=fname+' Magnitude',ytit=ylab2,symsiz=ss
            ...
            if ps then begin
               ...
               !p.multi=[0,1,4]
            endelse
            ploterror,-x[0,zgood]+refam,y[zgood]-yfit,oerr[zgood],psym=8, $
            ...
            ploterror,x[2,zgood]+refcolor,y[zgood]-yfit,oerr[zgood],psym=8, $
   ...
   ;            xtit='Point number',ytit=ylab2,charsize=cs,symsiz=ss
            ...
            if ps then begin
               ...
               y=y+dy
               xyouts,x,y,str,charsize=cs,/normal
               str=string('airmass^2               ',trans[5],pm,transig[5],transtr[5],format=fmt)
               y=y+dy
               xyouts,x,y,str,charsize=cs,/normal
               str=string('color^2                 ',trans[6],pm,transig[6],transtr[6],format=fmt)
               y=y+dy
               xyouts,x,y,str,charsize=cs,/normal
               if first then $
      ...
      if keyword_set(files) and keyword_set(histfile) and not (keyword_set(nosave)) then begin
