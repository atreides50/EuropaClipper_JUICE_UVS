...
;  2016/04/16, MWB, modified to use new rdsource.pro routine
...
pro showsrc,imname,srclist,reflist,OUTFILE=outfile,BINFAC=binfac, $
   ...
   self='showsrc: '
   if badpar(imname,[1,2,3,4,5,7,12],[0,2],caller=self+'(imname) ', $
                                         type=imtype) then return
   ...
   if badpar(windownum,[0,2,3],0,caller=self+'(WINDOWNUM) ', $
                                 default=7) then return
   if badpar(negative,[0,1,2,3],0,caller=self+'(NEGATIVE) ', $
                                 default=0) then return
   if badpar(lowsig,[0,2,3,4,5],0,caller=self+'(LOWSIG) ', $
                                 default=-3.0) then return
   if badpar(hisig,[0,2,3,4,5],0,caller=self+'(HISIG) ', $
                                 default=5.0) then return
   if badpar(srcmaglim,[0,2,3,4,5],0,caller=self+'(SRCMAGLIM) ', $
                                 default=99.0) then return

   if badpar(forcestretch,[0,1,2,3],0,caller=self+'(FORCESTRETCH) ', $
                                 default=0) then return
   ...
   plot,[0],[1],/nodata,xmargin=[0,0],ymargin=[0,0], $
      xr=[0,float(xsm)*sf-1],yr=[0,float(ysm)*sf-1],xstyle=5,ystyle=5,/noerase

   setusym,-1

   if srclist ne '' then begin
      rdsource,srclist,sinfo
      if size(sinfo,/type) eq 8 then begin
         z=where(sinfo.mag le srcmaglim,count)
         if count ne 0 then $
            plots,sinfo.xpos[z],sinfo.ypos[z],psym=4,color='00ff00'xl,symsize=2
      endif
   endif
