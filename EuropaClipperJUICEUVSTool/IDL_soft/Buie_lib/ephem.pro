...
pro ephem,in_jd,obs,code,object,ephemeris,DEBUG=debug,CACHE=cache, $
       FNCACHE=in_cachefile,PIPE=pipe,OBSPOS=obspos
   ...
   self='EPHEM: '
   if badpar(in_jd,5,[0,1],caller=self+'(jd) ' ,npts=npts) then return
   if badpar(obs,[1,2,3,7],[0,1],caller=self+'(obs) ', $
                                 npts=nobs,type=codetype) then return
   if badpar(code,2,0,caller=self+'(code) ') then return
   if badpar(object,7,[0,1], $
               caller=self+'(object) ',RANK=objrank,NPTS=nobj) then return

   if badpar(debug,[0,1,2,3],0,caller=self+'(DEBUG) ',default=0) then return
   if badpar(cache,[0,1,2,3],0,caller=self+'(CACHE) ',default=0) then return
   if badpar(in_cachefile,[0,7],0,caller=self+'(CACHEFILE) ', $
                                  default='') then return
   if badpar(obspos,[0,4,5],1,caller=self+'(OBSPOS) ', $
                              default=[0.,0.,0.]) then return
   ...
   if not goodcache then begin
      ...
      if n_elements(pipe) ne 0 then begin
         cache_pipe = 1
         ;if pipe not already opened, open it
   ...
   for i=0L,npts-1 do begin
      ...
      if goodcache then begin
         ...
         if debug then begin
            ...
            if obse eq 'G**' or obse eq 'S**' then $
               print,obspos
         endif

         printf,pipe,jd[i],obse,code,obj,format='(f13.5,1x,a3,1x,i2,1x,a)'
         if obse eq 'G**' or obse eq 'S**' then printf,pipe,obspos
         flush,pipe
         readf,pipe,line
