...
pro stats,data,ROBO=robo,SILENT=silent,TITLE=title,NBINS=nbins,XTITLE=xtitle, $
            MEAN=avg,charsize=charsize,WINDOW=window,IDX=idx,HIST=hist, $
            GAUSSIAN=gaussian
   ...
   if badpar(robo,[0,1,2,3],0,CALLER=self+'(ROBO) ',DEFAULT=0) then return
   if badpar(silent,[0,1,2,3],0,CALLER=self+'(SILENT) ',DEFAULT=0) then return
   if badpar(gaussian,[0,1,2,3],0,CALLER=self+'(GAUSSIAN) ',DEFAULT=0) then return
   ...
   xyouts,0.1*nbins*binsz+min_data,yr[0]+0.9*(yr[1]-yr[0]), $
      'Mean '+string(avg),/data,align=0.0,color=0,charsize=charsize
   xyouts,0.1*nbins*binsz+min_data,yr[0]+0.8*(yr[1]-yr[0]), $
      'sigma '+string(stddev),/data,align=0.0,color=0,charsize=charsize
   if gaussian then begin
      gauss1d,idx,avg,stddev,gauss,/sigma
      oplot,idx,gauss/total(gauss)*total(hist),color='0000ff'xl
   endif
