...
pro looker_display, state, NOLOAD=noload, NOSCORE=noscore
   ...
   if (*state).fnobj ne (*state).lastobj then begin
      ...
      if (*state).exttag ne '' then begin
         ...
         fits_read,fn1+ft,tmp,hdr1,exten=strb36((*state).exttag),/noscale
   ...
   if fn2 ne '' and (fn2 ne (*state).lastfn2 or (*state).idxfn2 eq -1) then begin
      ...
      if (*state).exttag ne '' then begin
         ...
         fits_read,fn+ft,tmp,hdr2,exten=strb36((*state).exttag),/noscale
      ...
      str=strn((*state).idxfn2+1)+' of '+strn((*state).nfiles-1)
      widget_control,(*state).sectxtid,set_value=str

   endif
   
   if fn2 eq '' then widget_control,(*state).sectxtid,set_value=''
   ...
   plot,[0],[1],/nodata,xmargin=[0,0],ymargin=[0,0], $
      xr=(*state).x0+[0,(*state).szx], $
      yr=(*state).y0+[0,(*state).szy], $
...
; if second frame is the third or later, use existing logic
; if second frame is the second, and curframe=1, and dist>50, start a
;    new object and let the click through.  if this second condition isn't
;    met the existing object is measured as expected.
      if (*state).idxfn2 ge 1 and dist gt 50.0 and  $
                                  oldx ge 0.0 and oldy ge 0.0 then begin
...
;      if dist gt 50.0 and oldx ge 0.0 and oldy ge 0.0 then begin
      ...
      endif else if (*state).idxfn2 eq 0 and dist gt 50.0 and $
                                oldx ge 0.0 and oldy ge 0.0 then begin
         (*state).curobj = (*state).nobj
         looker_objinfo,state
...
; previous
      ...
      'First Secondary': begin
         if (*state).newfile gt 0B then (*state).newfile=0B
         widget_control, (*state).fn1id, get_value=fn1
         if fn1 eq '' then return
         z=where(*(*state).fntrip ne fn1,count)
         if count eq 0 then return
         if (*state).idxfn2 eq 0 then return
         newstr=(*(*state).fntrip)[z[0]]
         widget_control, (*state).mainbase, UPDATE=0
         widget_control, (*state).fn2id,    SET_VALUE=newstr
         widget_control, (*state).mainbase, UPDATE=1
         (*state).idxfn2 = -1
...
;         idx = (*state).idxfn2*2
      ...
      'Next Secondary': begin
         if (*state).newfile gt 0B then (*state).newfile=0B
         widget_control, (*state).fn1id, get_value=fn1
         if fn1 eq '' then return
         z=where(*(*state).fntrip ne fn1,count)
         if count eq 0 then return
         if (*state).idxfn2 lt 0 then begin
            newstr=(*(*state).fntrip)[z[0]]
         endif else if (*state).idxfn2 eq count-1 then begin
            return
         endif else begin
            newstr=(*(*state).fntrip)[z[(*state).idxfn2+1]]
         endelse
         widget_control, (*state).mainbase, UPDATE=0
         widget_control, (*state).fn2id,    SET_VALUE=newstr
         widget_control, (*state).mainbase, UPDATE=1
      ...
      'Previous Secondary': begin
         ...
         if (*state).idxfn2 lt 0 then begin
            newstr=(*(*state).fntrip)[z[0]]
         endif else if (*state).idxfn2 eq 0 then begin
            return
         endif else begin
            newstr=(*(*state).fntrip)[z[(*state).idxfn2-1]]
         endelse
...
;print,(*state).curobj,(*state).nobj,(*state).idxfn2,(*state).nfiles
         if (*state).curobj ne (*state).nobj and $
             ((*state).idxfn2 eq 0 or (*state).nfiles le 2) then begin
      ...
      'Zoom': BEGIN
         ...
         case event.press OF
            ...
            4: begin
               ...
               ; nudge position
               if (*state).curframe eq 1 then begin
                  xpos = (*(*state).xyvals)[ 0, (*state).curobj ]
                  ypos = (*(*state).xyvals)[ 1, (*state).curobj ]
               endif else begin
                  xpos = (*(*state).xyvals)[ ((*state).idxfn2+1)*2, $
                                              (*state).curobj ]
                  ypos = (*(*state).xyvals)[ ((*state).idxfn2+1)*2+1, $
                                              (*state).curobj ]
               endelse

               xz = (xpos - (*state).zx0) * (*state).zf
               yz = (ypos - (*state).zy0) * (*state).zf

               dx = (event.x-xz)/(*state).szsx*4.0 ; maximum offset, 2 pixels
               dy = (event.y-yz)/(*state).szsx*4.0
...
PRO looker, KEYLIST=in_keylist, PATH=path, PSCALE=pscale, OBJRAD=objrad, $
   ...
   b1 = widget_base( cbase, /row, /frame)
   t1 = widget_label( b1, value=' 2nd:' )
   sec0id = widget_button( b1, VALUE=' 0 ', uvalue='First Secondary' )
   secpid = widget_button( b1, VALUE=' + ', uvalue='Next Secondary' )
   secnid = widget_button( b1, VALUE=' - ', uvalue='Previous Secondary' )
   sectxtid = widget_label(b1,value='',/dynamic_resize)
