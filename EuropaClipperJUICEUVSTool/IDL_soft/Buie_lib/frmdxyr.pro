...
;             in the individual frmdxdy.  See documentation for frmdxdy.pro
...
pro frmdxyr,in_x1,in_y1,in_x2,in_y2,thetamin,thetamax,dtheta, $
            ...
            SCALEFAC=scalefac,MAXABSOFFSET=maxabsoffset,FNDRAD=fndrad
   ...
   if badpar(maxabsoffset,[0,2,3,4,5],0,caller=self+'(MAXABSOFFSET) ', $
   ...
   for i=0,npts-1 do begin
      ...
      if error0 eq 0 and maxabsoffset gt 0 then begin
         if abs(xoff0) gt maxabsoffset or abs(yoff0) gt maxabsoffset then $
            error0=8
      endif
      verror[i] = error0
   ...
   if min(verror) ge 1 then begin
      xoff=0.
      yoff=0.
      theta=0.
      if not silent then print,'No frmdxdy calls worked.'
      error=1
      return
   endif

   ; Find the best angle
   zg=where(verror eq 0)
   z=where(fom[zg] eq max(fom[zg]))
   ibest = zg[z[0]]
   ...
   if ibest eq 0 or ibest eq npts-1 then begin
      ...
      yoff=0.
      theta=0.
      if not silent then print,'Best is at the edge of the search region.'
      error=1
      return
   endif
   ...
   while delta gt tolerance do begin
      ...
      y1r = -(x1-nx/2)*sin(angle_r)+(y1-ny/2)*cos(angle_r) + ny/2
      frmdxdy,x1r,y1r,x2,y2,xoff0,yoff0,error0,nx=nx,ny=ny,fom=fom0, $
              SPATIAL=spatial,INDEX=indx,scalefac=scalefac,fndrad=fndrad
      index3[1,*]=indx
      xoff[1] = xoff0
      yoff[1] = yoff0
      fom[1]  = fom0
