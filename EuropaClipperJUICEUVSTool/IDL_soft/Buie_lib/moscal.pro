...
pro moscal,root,outdir,start,nframes,bias,dark,flat, $
   EXCLUDE=exclude,SCALE=in_scale,EXPKEY=expkey,SKIP=skip, $
   ...
   if badpar(doall,[0,1,2,3],0,caller=self+'(DOALL) ',default=0) then return
   ...
   if countg eq 0 then begin
      print,'all frames are excluded, nothing to do.'
      return
   endif
   ...
   for i=0,nframes-1 do begin
      if frames[i] lt 0 then continue
      fname=root+string(frames[i],format=fnfmt)
      if skip then $
         if exists(outdir+fname+ft) then frames[i]=-1
      hdr=headfits(fname+ft)
      obstype=strtrim(strupcase(sxpar(hdr,'OBSTYPE')),2)
      if obstype ne 'OBJECT' and not doall then frames[i]=-1
   endfor

   zg=where(frames ne -1,countg)
   if countg eq 0 then begin
      print,'no frames that need to be processed, nothing to do.'
      return
   endif
