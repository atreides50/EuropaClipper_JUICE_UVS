...
pro render,map,radius,in_scale,in_pole,selat,selon,sslat,sslon,nx,ny,image, $
   ...
   if badpar(silent,[0,1,2,3],0,caller=self+'(SILENT) ', $
                default=0) then return
   if badpar(in_xoffset,[0,1,2,3,4,5],0,caller=self+'(XOFFSET) ', $
                default=0.0) then return
   if badpar(in_yoffset,[0,1,2,3,4,5],0,caller=self+'(YOFFSET) ', $
                default=0.0) then return
   if badpar(pfn,[0,7],0,caller=self+'(PFN) ',default='') then return

   nx = round(nx)
   ny = round(ny)

   ; IDL v8.1 crashes if double allowed to pass beyond this point.
   xoffset=float(in_xoffset)
   yoffset=float(in_yoffset)
      ...
      if count_zon gt 0 then begin
         ...
         if ill_count gt 0 then begin
            ...
            ; Hapke scattering function.
               ...
               image[zon[ill]] = bidr(map[xidx[ill],yidx[ill]], $
                                      mu[ill],smu[ill],g[ill], $
                                      hapke[0],hapke[1])*!pi
            ...
            ; Hapke scattering function, second generation.
               ...
               if pfn eq '' then begin
                  image[zon[ill]] = bidr2(map[xidx[ill],yidx[ill]], $
                                          mu[ill],smu[ill],g[ill], $
                                          hapke2[0],hapke2[1],hapke2[2], $
                                          hapke2[3],h93=h93)*!pi
               endif else begin
                  pparms=n_elements(hapke2)-3
                  image[zon[ill]] = bidr2(map[xidx[ill],yidx[ill]], $
                                          mu[ill],smu[ill],g[ill], $
                                          hapke2[0],hapke2[1:pparms], $
                                          hapke2[pparms+1],hapke2[pparms+2], $
                                          pparms=pparms,h93=h93,pfn=pfn)*!pi
               endelse
         ...
         if pfn eq '' then begin
            ...
            image[geom[zon*illc:(zon+1)*illc-1]] = $
               bidr2(map[geom[xidx*illc:(xidx+1)*illc-1], $
                     ...
                     geom[g*illc:(g+1)*illc-1], $
                     hapke2[0],hapke2[1:pparms], $
                     hapke2[pparms+1],hapke2[pparms+2],pfn=pfn,h93=h93)*!pi
         endelse
