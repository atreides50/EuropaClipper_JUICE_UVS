...
;-
pro jitterk,in_dt,in_dx,in_dy,exptime,kernel, $
            SUBSAMPLE=subsample,DTSAMPLE=dtsample, $
            MOVING=moving,EXPCENTER=expcenter

   self='jitterk: '
   if badpar(in_dt,[2,3,4,5],1,caller=self+'(dt) ',npts=ndt) then return
   if badpar(in_dx,[2,3,4,5],1,caller=self+'(dx) ',npts=ndx) then return
   if badpar(in_dy,[2,3,4,5],1,caller=self+'(dy) ',npts=ndy) then return
   if badpar(exptime,[2,3,4,5],0,caller=self+'(exptime) ') then return
   if badpar(subsample,[0,2,3],0,caller=self+'(SUMSAMPLE) ', $
                                 default=1) then return
   if badpar(dtsample,[0,2,3,4,5],0,caller=self+'(DTSAMPLE) ', $
                                    default=0.1) then return
   if badpar(moving,[0,1,2,3],0,caller=self+'(MOVING) ',default=0) then return
   if badpar(expcenter,[0,1,2,3],0,caller=self+'(EXPCENTER) ', $
   ...
   if expcenter then begin
      ; center on the position at the middle of the exposure
      interp,dt,dx,exptime/2.0,xmid
      interp,dt,dy,exptime/2.0,ymid
      xvec  = xvec-trimrank(xmid)*subsample
      yvec  = yvec-trimrank(ymid)*subsample
   endif else begin
...
;oplot,dx*subsample-xmean,dy*subsample-ymean,psym=-4,color='0000ff'xl
   ...
   xdw=max(abs(round(xvec))) > 1
   ydw=max(abs(round(yvec))) > 1
