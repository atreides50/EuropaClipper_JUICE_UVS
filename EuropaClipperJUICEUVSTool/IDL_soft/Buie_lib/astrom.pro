...
;; Commented out, MWB, 2015/08/12, this isn't called by anyone.  It might
;;   be useful but has problems after the ref/opt scrub.
...
;;              raref  - ra of optical axis
;;              decref - dec of optical axis
...
pro astrom_twostar,dv,linfo,sra,sdec,sx,sy,ssz,valid
   ...
   linfo_orig = linfo
;print,'original linear coordinate transformation'
;help,linfo
...
retry:
   ...
   ; Using current linear solution, compute predicted star locations
   ; simple linear conversion, xi,eta returned will be with respect to
   ;   raref,decref in the linear info structure.  xi,eta will be in radians
   astrd2xy,sra,sdec,linfo,sx,sy,XI=sxi,ETA=seta

   ; Display the image with an overlay showing the predicted star locations
   astrom_dim,dv.win,dv.bim,dv.binfac,dv.border
   for i=0L,n_elements(sx)-1 do $
      oplot,[sx[i]],[sy[i]],psym=4,color=color2,symsize=ssz[i]

   ; Select first star from catalog list
...
print,'Click at ',xs1,ys1
   ...
   oldy = sy[zs1]
   rastr,sra[zs1],1,ras
   decstr,sdec[zs1],0,decs
print,'First anchor star predicted at (x,y)     ',oldx,oldy
print,'First anchor star actual coordinates are ',ras,' ',decs
   ...
   ; compute the ra,dec of this position according to the current linfo
   astxy2rd,xs1m,ys1m,linfo,sra_old,sdec_old
   rastr,sra_old,1,ras
   decstr,sdec_old,0,decs
print,'Actual position of star (x,y)',xs1m,ys1m
print,'position difference is       ',xs1m-oldx,ys1m-oldy
print,'ra,dec of new position according to the old solution ',ras,' ',decs

print,'linfo ref',linfo.raref,linfo.decref
print,'offset ',( sra[zs1] - sra_old),(sdec[zs1] - sdec_old)

   ; Using first star, update the reference position (raref,decref)
   linfo.raref  = linfo.raref  + ( sra[zs1] - sra_old)
   linfo.decref = linfo.decref + (sdec[zs1] - sdec_old)

   ; recompute predicted positions and xi,eta based on update from first star
   ;   and then update the plot if it changed enough to matter
   astrd2xy,sra,sdec,linfo,sx,sy,XI=sxi,ETA=seta
   if sqrt((oldx-xs1m)^2+(oldy-ys1m)^2) gt 1.0 then begin
      astrom_dim,dv.win,dv.bim,dv.binfac,dv.border
      for i=0L,n_elements(sx)-1 do $
         oplot,[sx[i]],[sy[i]],psym=4,color=color2,symsize=ssz[i]
      oplot,[xs1m],[ys1m],psym=4,color=color1,symsize=5
   ...
   oldy = sy[zs2]
   rastr,sra[zs2],1,ras
   decstr,sdec[zs2],0,decs
;print,'Second anchor star at (x,y)',oldx,oldy,' ',ras,' ',decs
   ...
   dy      = linfo.yflip*(ys2m-ys1m)
   pixsep = sqrt(dx^2+dy^2)
;print,'offset from pair (dx,dy)',dx,dy,', pixel distance between is ',pixsep

   ; standard coordinates of the two stars from the current solution
   xi1  = sxi[zs1]  * 180.0d0/!dpi*3600.0d0
   eta1 = seta[zs1] * 180.0d0/!dpi*3600.0d0
   xi2  = sxi[zs2]  * 180.0d0/!dpi*3600.0d0
   eta2 = seta[zs2] * 180.0d0/!dpi*3600.0d0

;print,'Star 1 at xi,eta ',xi1,eta1
;print,'Star 2 at xi,eta ',xi2,eta2
   angsep = sqrt((xi2-xi1)^2+(eta2-eta1)^2)
;print,'angular distance between the pair is ',angsep,' arcsec'

   ; plate scale in arcsec/pixel and image rotation angle.
   pscale  = angsep/pixsep
   rang    = atan(dy*(xi2-xi1)-dx*(eta2-eta1),dx*(xi2-xi1)+dy*(eta2-eta1))
   print,'New plate scale is ',pscale,' arcsec/pixel'
   print,'New rotation angle ',rang,' radians'

   ; Save values
   linfo.pscale = pscale
   linfo.rang   = rang

   ; recompute predicted location of star 1
   astxy2rd,xs1m,ys1m,linfo,sra_old,sdec_old
   ; Using first star, update the reference position again
   linfo.raref  = linfo.raref  +  sra[zs1] - sra_old
   linfo.decref = linfo.decref + sdec[zs1] - sdec_old

   rastr,linfo.raref,1,ras
   decstr,linfo.decref,0,decs
   print,'New reference position for chip ',ras,' ',decs

;astrd2xy,linfo.raref,linfo.decref,linfo,xref_chk,yref_chk
;print,'computed position of reference position',xref_chk,yref_chk

   ; re-do prediction on last update and replot
   astrd2xy,sra,sdec,linfo,sx,sy,XI=sxi,ETA=seta
   astrom_dim,dv.win,dv.bim,dv.binfac,dv.border
   for i=0L,n_elements(sx)-1 do $
      oplot,[sx[i]],[sy[i]],psym=4,color=color2,symsize=ssz[i]
   oplot,[xs1m],[ys1m],psym=4,color=color1,symsize=5
   oplot,[xs1m],[ys1m],psym=4,color=color1
   oplot,[xs2m],[ys2m],psym=4,color=color1,symsize=5
   oplot,[xs2m],[ys2m],psym=4,color=color1
...
pro astrom,root,fileno, $
   ...
   if badpar(twostar,[0,1,2,3],0,CALLER='ASTROM: (TWOSTAR) ', $
                              default=0) then return
   if badpar(nofwhmfilter,[0,1,2,3],0,CALLER='ASTROM: (NOFWHMFILTER) ', $
                              default=0) then return
...
;print,'E goodsolution=',goodsolution
         ...
         ; xcref,ycref is commonly the center of the actual image.
         ;   but is the reference point for the pointing of the image.
         linfo = { $
            ...
            xcref:  xcenter, $    ; X reference location
            ycref:  ycenter }     ; Y reference location

         if not found_center then begin
            ; Compute the RA,Dec of the reference with linear approximation
            astxy2rd,linfo.xcref,linfo.ycref,linfo,raref,decref
            linfo.raref  = raref
            linfo.decref = decref
         endif
         ...
         if doplast then begin
            ...
            for i=0,numobj-1 do begin
               ...
               print,i,pastname[i],str1,str2,str3,asterr[0,i]*!radeg*3600.0,'"', $
                  ...
                  format='(i3,1x,a15,1x,a,1x,a,2x,a,f7.1,a,2x,f4.1,a,f4.1)'
            endfor
         endif
...
;   for this to work.
            if not twostar then begin
               ...
               if doshells and ( maxrcrit lt 0 or (maxrcrit gt 0 and $
                  ...
                  rper = shells[0]
                  r = sqrt((xpos-linfo.xcref)^2 + (ypos-linfo.ycref)^2)
                  zr = sort(r)
                  nr = long(n_elements(r)*rper)
                  r2 = sqrt((sx-linfo.xcref)^2 + (sy-linfo.ycref)^2)
                  zr2 = where(r2 le max(r[zr[nr-1]])*1.01 and $
                              sx ge -dv.nx/2*0.10  and $
                              sx le  dv.nx/2*2.10  and $
                              sy ge -dv.ny/2*0.10  and $
                              sy le  dv.ny/2*2.10, count)
               ...
               frmdxdy,sx[zr2],sy[zr2],xpos[zr],ypos[zr],xoff,yoff,xyerror, $
                  nx=sz[1],ny=sz[2],scalefac=scalefac,fndrad=fndrad
               ...
               if xyerror gt 0 then begin
                  frmdxyr,sx[zr2],sy[zr2],xpos[zr],ypos[zr], $
                     ...
                     tolerance=rotscan[2],scalefac=scalefac, $
                     maxabsoffset=100.0,fndrad=fndrad
            ...
            if twostar or xyerror eq 1 then begin
               ...
               if headercoord then begin
                  raoff=linfo.raref-hra
                  if raoff gt      !pi then raoff=raoff-2.0d0*!dpi
                  if raoff lt -1.0*!pi then raoff=raoff+2.0d0*!dpi
                  raoff=raoff*cos(linfo.decref)
                  decoff=linfo.decref-hdec
...
restart_shell:
                  ...
                  dx = (xpos[sidx[zy]]-linfo.xcref)/renormfac
                  dy = (ypos[sidx[zy]]-linfo.ycref)/renormfac
                  ; Convert from ra,dec to xi,eta
                  astrd2sn,sra,sdec,linfo.raref,linfo.decref,xi,eta,/ARCSEC
                  ...
                  tinfo={renormfac:renormfac,cxi:cxi,ceta:ceta,terms:tterms, $
                         xcref:linfo.xcref,ycref:linfo.ycref, $
                         raref:linfo.raref,decref:linfo.decref}
...
print,fnsrc
      ...
      retry:
            ...
            rastr,linfo.raref,1,ras1
            decstr,linfo.decref,0,decs1
...
;photographic refinement
         ...
         if nofwhmfilter then begin
            zz=where(fwhm le 0.7,countzz)
         endif else begin
            zz=where(abs(fwhm-meanfwhm) gt 3.0*fwhmstddev or $
                                                     fwhm le 0.7,countzz)
         endelse
         ...
         renormfac=sqrt(double(sz[1])^2+double(sz[2])^2)
         dx = (xpos-linfo.xcref)/renormfac
         dy = (ypos-linfo.ycref)/renormfac
         ...
         if extlist[ext] eq 0 then begin
            ...
            if ext eq 0 then begin
               ...
               fitdata[ext].xcref     = linfo.xcref
               fitdata[ext].ycref     = linfo.ycref
               fitdata[ext].raref     = linfo.raref
               fitdata[ext].decref    = linfo.decref
         ...
         dx = (xpos[zg]-linfo.xcref)/renormfac
         dy = (ypos[zg]-linfo.ycref)/renormfac
...
;         if exists(reffile) ne 1 or twostar then begin
         ...
         if ze eq -1 and zx eq -1 then begin
            ...
            t_xc        = [t_xc,        linfo.xcref,linfo.xcref]
            t_yc        = [t_yc,        linfo.ycref,linfo.ycref]
            t_prot      = [t_prot,      0.0,0.0]
            t_renormfac = [t_renormfac, renormfac,renormfac]
            t_cra       = [t_cra,       linfo.raref,linfo.raref]
            t_cdec      = [t_cdec,      linfo.decref,linfo.decref]
            ...
            t_cra[ze]        = linfo.raref
            t_cdec[ze]       = linfo.decref
            t_photzp[ze]     = magdiff
            ...
            t_cra[zx]        = linfo.raref
            t_cdec[zx]       = linfo.decref
            t_photzp[zx]     = magdiff
...
; photographic refinement
         ...
         if spot[0] gt -0.9e9 then begin
            ...
            for sn=0,nspots-1 do begin
               xi = asteval((spot[0,sn]-linfo.xcref)/renormfac, $
                            (spot[1,sn]-linfo.ycref)/renormfac, $
                            cxi,terms)/3600.0d0*!dpi/180.0d0
               eta= asteval((spot[0,sn]-linfo.xcref)/renormfac, $
                            (spot[1,sn]-linfo.ycref)/renormfac, $
                            ceta,terms)/3600.0d0*!dpi/180.0d0
               astsn2rd,xi,eta,linfo.raref,linfo.decref,ra,dec
               rastr,ra,4,ras
               decstr,dec,3,decs
...
; photographic refinement
                        ...
                        xi = asteval((xm-linfo.xcref)/renormfac, $
                                     (ym-linfo.ycref)/renormfac, $
                                     cxi,terms)/3600.0d0*!dpi/180.0d0
                        eta= asteval((xm-linfo.xcref)/renormfac, $
                                     (ym-linfo.ycref)/renormfac, $
                                     ceta,terms)/3600.0d0*!dpi/180.0d0
                        astsn2rd,xi,eta,linfo.raref,linfo.decref,ra,dec
                        rastr,ra,4,ras
...
;                     oplot,astx,asty,psym=8,color=color1,symsize=0.2
               ...
               if roam then begin
                  ...
                  repeat begin
                     ...
                     if !mouse.button eq 0 then begin
                        xi = asteval((x-linfo.xcref)/renormfac,(y-linfo.ycref)/renormfac, $
                                     cxi,terms)/3600.0d0*!dpi/180.0d0
                        eta= asteval((x-linfo.xcref)/renormfac,(y-linfo.ycref)/renormfac, $
                                     ceta,terms)/3600.0d0*!dpi/180.0d0
                        astsn2rd,xi,eta,linfo.raref,linfo.decref,ra,dec
                        rastr,ra,4,ras
                        decstr,dec,3,decs
               ...
               if !mouse.button eq 1 then begin
                  ...
                  xi = asteval((xm-linfo.xcref)/renormfac,(ym-linfo.ycref)/renormfac, $
                               cxi,terms)/3600.0d0*!dpi/180.0d0
                  eta= asteval((xm-linfo.xcref)/renormfac,(ym-linfo.ycref)/renormfac, $
                               ceta,terms)/3600.0d0*!dpi/180.0d0
                  astsn2rd,xi,eta,linfo.raref,linfo.decref,ra,dec
                  rastr,ra,4,ras
                  decstr,dec,3,decs
...
;print,'F countgood=',countgood
         ...
         astxy2rd,linfo.xcref,linfo.ycref,fitdata[ext],newraref,newdecref,/full
         astcvt,'mn',0.,0.,fitdata[ext],'rd',newraref,newdecref
         ; Save the final center
         astrom_svctr,thisexp,newraref,newdecref,linfo,hdrinfo,outdir=outdir
