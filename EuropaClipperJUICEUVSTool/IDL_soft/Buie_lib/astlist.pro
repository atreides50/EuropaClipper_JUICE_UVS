...
PRO astlist,file,outfile,slowfile,DESOBJ=desobj
   ...
   if badpar(outfile,[0,7],0,CALLER='ASTLIST: (outfile) ', $
                             default='[none]') then return
   if badpar(slowfile,[0,7],0,CALLER='ASTLIST: (slowfile) ', $
                              default='[none]') then return
...
;   idx=sort(elong)
   ...
   if outfile ne '[none]' then begin
      ...
      printf,lun,title
      for i=0,nobj-1 do begin
         if desflag[idx[i]] eq 0B then $
            printf,lun,object[idx[i]]+blanks,ffn[idx[i]], $
               count[idx[i]],avgmag[idx[i]],magerr[idx[i]], $
               rate[idx[i]],objdir[idx[i]],dis[idx[i]],dt[idx[i]], $
               rsig[idx[i]],elong[idx[i]],range[idx[i]], $
               format=fmt
      endfor
      free_lun,lun
   endif
