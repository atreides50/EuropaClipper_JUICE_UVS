...
;                     vectors.
;-
pro photphot,img,x0,y0,in_dw, $
         xcen,ycen,xfwhm,yfwhm,xflux,yflux,xback,yback, $
         xcenerr,ycenerr,xfluxerr,yfluxerr, $
         SKY=in_sky,DEBUG=debug,VERBOSE=verbose,SILENT=silent

   self = 'PHOTPHOT: '
   if badpar(img,[1,2,3,4,5],2,caller=self+'(img) ') then return
   if badpar(x0,[2,3,4,5],[0,1],caller=self+'(x0) ',npts=nx0) then return
   if badpar(y0,[2,3,4,5],[0,1],caller=self+'(y0) ',npts=ny0) then return
   if badpar(in_dw,[1,2,3,4,5],[0,1],caller=self+'(dw) ',npts=ndw) then return
   if badpar(in_sky,[0,1,2,3,4,5],[0,1],caller=self+'(SKY) ', $
                                        npts=nsky) then return
