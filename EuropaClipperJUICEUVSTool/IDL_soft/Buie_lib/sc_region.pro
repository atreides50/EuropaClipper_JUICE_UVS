...
pro sc_region, db, ra, dec, radius, tmptbl, $
   ...
   if badpar(mag_max,[0,1,2,3,4,5,6,9],0,caller=self+'(MAG_MAX) ', $
            default=-3.0) then return
   if badpar(mag_min,[0,1,2,3,4,5,6,9],0,caller=self+'(MAG_MIN) ', $
            default=12.0) then return
   if badpar(sclass,[0,7],0,caller=self+'(SCLASS) ', $
            default='') then return
   ...
   if decmax GE PI_2 then begin
      ...
      if decmin LE -PI_2 then begin
         ...
         delra = radius/cos(maxdec)
         if delra lt !pi then begin
