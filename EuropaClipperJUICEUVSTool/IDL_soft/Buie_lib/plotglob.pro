...
pro plotglob,plon0,plat0,p0, $
      ...
      LINESTYLE=linestyle,offset=offset
   ...
   if badpar(offset,[0,1,2,3,4,5],1,CALLER='PLOTGLOB: (OFFSET) ',default=[0,0]) then return
   ...
   for i=0,n_elements(dolat)-1 do begin
      ...
      oplot,yppp+offset[0],zppp+offset[1],max_value=2.0,color=colors[2], $
         linestyle=linestyle,/noclip
   endfor
   ...
   notvis=where(xppp lt 0)
   if (notvis[0] ne -1) then zppp[notvis] = 10.0
   oplot,yppp+offset[0],zppp+offset[1],max_value=2.0,thick=2.0, $
      color=colors[0],linestyle=linestyle,/noclip
   ...
   if (notvis[0] ne -1) then zppp[notvis] = 10.0
   oplot,yppp+offset[0],zppp+offset[1],max_value=2.0,thick=2.0, $
      color=colors[1],linestyle=linestyle,/noclip
   ...
   if not nolimb then begin
      ...
      x = cos(angle)
      y = sin(angle)
      oplot,x+offset[0],y+offset[1],color=colors[3],linestyle=linestyle,/noclip
   endif
