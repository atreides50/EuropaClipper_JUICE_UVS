...
;  mbeta - this 3D array contains the
...
;  mgamma - this 2D array contains the
;          total(I_k*(R_k x K_q))
...
;  2016/04/16, SFH, corrected beta and gamma matrix generation to match
;    Miller algorithm, previous version was not correct for degree > 0
;-
pro ois_coef,iratio,img,ref,deg,kernsz,nbasis,kbasis, $
       xstamp,ystamp,wstamp,ibasis,malpha,mbeta,mgamma,pmax

   ; Define the Arrays
   pmax=n_elements(wstamp)
   malpha=fltarr(pmax,deg+1,deg+1)
   mbeta=fltarr(pmax,nbasis,nbasis)
   mgamma=fltarr(pmax,nbasis)

   for u=0,deg do $
      for v=0,deg-u do $
         malpha[*,u,v]=xstamp^u*ystamp^v
   ...
   for k=0,pmax-1 do begin
      ...
      if ibasis eq 'DELTA' then begin
         ...
         if iratio eq 'NO' then begin
            ...
            ckbasis[0:(kernsz^2-1)/2,*,*]= $
                shift(ckbasis[0:(kernsz^2-1)/2,*,*],[1,0,0])
            ckbasis[0,*,*]=rstamp
         endelse
      endif else begin
         ; gaussian
      ...
      ; C matrix from Miller 2008 PASP paper
      for n=0,nbasis-1 do $
         for q=0,nbasis-1 do $
            mbeta(k,n,q)=total(ckbasis(n,*,*)*ckbasis(q,*,*))

      ; D matrix from Miller 2008 PASP paper
      for q=0,nbasis-1 do $
         mgamma(k,q)=total(istampr(*,*)*ckbasis(q,*,*))

      ; Old code that isn't correct, retained here as a reminder for
      ;   speed up if needed.
...
pro ois_filtpix,kernwd,image,mask,cimage,skymean,skysig,SEED=seed
   ...
   self='ois_filtpix: '
   if badpar(seed,[0,2,3,4,5,12,13,14,15],[0,1],caller=self+'(SEED) ') then return
