...
pro gettran,inst,date,filter,color1,color2,tran,transig,jdref, $
    refcolor,refam,found,fitted,nobs,chi2,quality, $
    NODB=nodb,DATABASE=database,TABLENAME=tablename,PATH=path, $
   ...
   if nodb then begin
      print,self,'WARNING!  Non-database functionality is deprecated.'
      histname=path+'transf_'+fil+'.'+c1+'m'+c2

      if not exists(histname) then begin
         if silent ne 0  then $
            print,self,'ERROR! Transformation file ',histname,' not found'
      ...
      if str1 ne tag then begin
         if silent ne 0 then $
            print,self,'ERROR! ',sfnames[filter], $
      ...
      tran     = [v1,v2,v3,v4,v5,0.,0.]
      transig  = [e1,e2,e3,e4,e5,0.,0.]
      jdref    = r1
      refcolor = 0.0
      refam    = 0.0
      ...
      cmd=['select instrument,RunDate,filter,color1,color2,k,k2,kcolor,', $
                  ...
                  'refcolor,refam,airsq,colsq,e_airsq,e_colsq,', $
                  'f_k,f_k2,f_kcolor,f_zeropt,f_ktime,f_airsq,f_colsq,', $
                  'nobs,chi2,quality' + $
                   ' from' + ' ' +  transfdbhook[1], $
      ...
      if verbose then print,self,'query on ', transfdbhook, cmd
      mysqlquery,dblun,cmd, dbinst,dbdate,dbfilt,dbcolor1,dbcolor2, $
                 t0,t1,t2,t3,t4,ts0,ts1,ts2,ts3,ts4, $
                 jdref,refcolor,refam,t5,t6,ts5,ts6, $
                 fs0,fs1, fs2,fs3, fs4, fs5,fs6, $
                 nobs,chi2, quality, $
                 format='a,a,a,a,a, f,f,f,f,f, f,f,f,f,f, d, f,f,f,f,f,f,'+ $
                        'a,a,a,a,a,a,a, i,f,a'
      free_lun, dblun

      fs = [fs0,fs1,fs2,fs3,fs4,fs5,fs6]
      tran = [t0,t1,t2,t3,t4,t5,t6]
      transig = [ts0,ts1,ts2,ts3,ts4,ts5,ts6]
      jdref = jdref[0]
...
;               ') transformation for [',tag,'] has internal db errors'
      ...
      if count gt 0 then $
         if silent ne 0  then $
            print,self,'ERROR! values in fitted:', fs[i]
