...
pro ltcrv,stand,fil,jd,am,serial,mag,err,object,objno,comp,dofil, $
       ...
       REFCOLOR=refcolor,REFAM=refam,AMSQ=amsq,COLSQ=colsq, $
   ...
   if badpar(in_cterm,[0,4,5],1,caller=self+'(CTERM)',npts=n9, $
                                default=[999.,0.]) then return
   if badpar(amsq,[0,4,5],1,caller=self+'(AMSQ)', $
                                default=[0.,0.]) then return
   if badpar(colsq,[0,4,5],1,caller=self+'(COLSQ)', $
                                default=[0.,0.]) then return
   if badpar(refcolor,[0,4,5],0,caller=self+'(REFCOLOR)', $
                                default=0.0) then return
   if badpar(refam,[0,4,5],0,caller=self+'(REFAM)', $
                                default=0.0) then return
   ...
   repeat begin
      ...
      starmag = mag[star] $
                - k2[0]*(scolor[0]-refcolor)*(am[star]-refam) $
                + cterm[0]*(scolor[0]-refcolor)
                + colsq[0]*(scolor[0]-refcolor)^2
                + amsq[0]*(am[star]-refam)^2

; needs extra terms propagated in here.
      starerr = sqrt( err[star]^2 + (k2[1]*(scolor[0]-refcolor)*(am[star]-refam))^2 $
                                  + (k2[0]*(scolor[1]-refcolor)*(am[star]-refam))^2 $
                                  + (cterm[1]*(scolor[0]-refcolor))^2 $
                                  + (cterm[0]*(scolor[1]-refcolor))^2 )
...
print,self,'This code is incomplete.  Do you really need to use it?'
stop
