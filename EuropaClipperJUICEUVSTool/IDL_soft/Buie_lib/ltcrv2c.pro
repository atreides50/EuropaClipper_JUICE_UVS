...
pro ltcrv2c,stand,fil,jd,time,am,serial,inst,instsig, $
   ...
   error=1
   self='LTCRV2C: '
   if badpar(stand, 7,        1,caller=self+'(stand) ') then return
   if badpar(fil,   7,        1,caller=self+'(fil) '  ) then return
   if badpar(jd,    5,        1,caller=self+'(jd) '   ) then return
   if badpar(time,  [4,5],    1,caller=self+'(time) ' ) then return
   if badpar(am,    [4,5],    1,caller=self+'(am) '   ) then return
   if badpar(serial,[1,2,3],  1,caller=self+'(serial) ') then return
   if badpar(inst,  [4,5],    1,caller=self+'(inst) ' ) then return
   if badpar(instsig,[4,5],   1,caller=self+'(instsig) ') then return
   if badpar(doobj, 7,        0,caller=self+'(doobj) '         ) then return
   if badpar(doser, [1,2,3],  0,caller=self+'(doser) '         ) then return
   if badpar(dofil, 7,        1,caller=self+'(dofil) '         ) then return
   if badpar(tran,  [4,5],    2,caller=self+'(tran) '   ) then return
   if badpar(transig,[4,5],   2,caller=self+'(transig) ') then return
   if badpar(jdref, 5,        1,caller=self+'(jdref) '  ) then return
   if badpar(refcolor, [4,5], 1,caller=self+'(refcolor) ') then return
   if badpar(refam, [4,5],    1,caller=self+'(refam) ') then return
   if badpar(color,[0,1,2,3,4,5],0,caller=self+'(color) ', $
                                default=refcolor) then return
   if badpar(colorsig,[0,1,2,3,4,5],0,caller=self+'(colorsig) ', $
                                default=0.) then return
   if badpar(filtname,[0,7],  1,caller=self+'(filtname) ', $
                                default=['default','default']) then return
   if badpar(file,  [0,7],    1,caller=self+'(file) ', $
                                default='no save') then return
   if badpar(epsilon,[0,1,2,3,4,5],0,caller=self+'(EPSILON) ', $
