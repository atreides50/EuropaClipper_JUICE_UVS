...
pro linkobj2,tag,files, $
       ...
       MAXOBJ=maxobj,SNRTHRESH=snrthresh,SRDDIR=srddir
   ...
   IF badpar(snrthresh,[0,2,3,4,5],0,CALLER=self+'(SNRTHRESH) ', $
                                     DEFAULT=-1) THEN return
   if badpar(srddir,[0,7],0,caller=self+'(SRDDIR) ',default='') then return

   if path ne '' then path=addslash(path)
   if srddir ne '' then srddir=addslash(srddir)
   ...
   for i=0,nfiles-1 do begin
      ...
      ; Get the list
      if not exists(srddir+files[i]) then begin
         print,'Unable to locate source list: ',srddir+files[i]
         goto,cleanup
      endif
      list = readfits(srddir+files[i],hdrsrc,/silent)
      idx=sort(list[*,3])
      ...
      for j=i+1,nfiles-1 do begin
         ...
         if countm ne 0 and countc ne 0 then begin
            ...
            if n_elements(ind1) gt 0 then begin
               ...
               if mean(tdx) gt maxoffset or mean(tdy) gt maxoffset then begin
                  str=string(files[i],files[j],mean(tdx),mean(tdy), $
                      format='(a,1x,a,1x,"  star offset too high",2(1x,f10.2))')
                  logerror,'linkobj2.err',str,caller=self
;                  goto,badoffset
               endif
...
quitloop:
   ...
   if maxobj gt 0 and nobj eq maxobj then begin
      str=files[0]+' '+files[1]+' more than '+strn(maxobj)+' objects.'
      logerror,'linkobj2.err',str,caller=self
   endif
...
badoffset:
   ...
   if killsrd then begin
      str='Deleting '+srddir+files[i]+' '+srddir+files[j]
      logerror,'linkobj2.err',str,caller=self
      file_delete,srddir+files[i],/quiet,/noexpand_path
      file_delete,srddir+files[j],/quiet,/noexpand_path
   endif
