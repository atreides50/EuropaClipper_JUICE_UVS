...
;  2016/06/24, MWB, added some extra optional plot output
;-
pro seeing,image,fwhm,objrad,nstars,ERROR=error, $
   ...
   if not nodisplay then begin
      print,n_elements(x),' sources found in initial pass with find.'
      stats,image[fix(x+0.5),fix(y+0.5)],window=2
   endif
   ...
   if not nodisplay then begin
      print,count,' sources below maxsignal threshold'
   endif
