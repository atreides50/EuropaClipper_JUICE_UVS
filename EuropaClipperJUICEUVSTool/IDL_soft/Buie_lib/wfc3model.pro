...
;             DISTORT=distort, $
   ...
   if badpar(jitter_in,   [0,4,5], [0,2],   caller=self+'(JITTER) ', $
                                        default=0.0,rank=jit_rank) then return
...
;   endif
   ...
   if jit_rank eq 0 then begin
      if jitter_in gt 0.0 then begin
         ...
         kernel=reverse(kernel,1) ; reverse on first axis
         kernel=reverse(kernel,2) ; reverse on second axis
         image=convol(image, kernel)
      endif
   endif else begin
      kernel=jitter_in/total(jitter_in)
      if verbose then help,kernel
      kernel=reverse(kernel,1) ; reverse on first axis
      kernel=reverse(kernel,2) ; reverse on second axis
      image=convol(image, kernel)
