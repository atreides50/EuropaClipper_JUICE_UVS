...
print,'Image ',imagelist[jj],'  ',strn(jj+1),' out of ',strn(nimages)
         ...
         srcx=trimrank(data[*,0])
         srcy=trimrank(data[*,1])
         ...
         ; Compute proper instrumental magnitudes for all sources
         basphote,gain,image,hdrinfo.exptime,srcx,srcy,rad,sky1,sky2, $
            /silent,/nolog,/exact,mag=inst0,err=instsig0,rdnoise=rdnoise
   ...
   gettran,'Nasacam',rundate,filter[0],filter[0],filter[1], $
      trans1,trsig1,jdref1,refcolor1,refam1,found,fitted,nobs,chi2,quality
   if not found or quality ne 'good' then begin
      print,self,'Error getting transformation'
      return
   endif

   gettran,'Nasacam',rundate,filter[1],filter[0],filter[1], $
      trans2,trsig2,jdref2,refcolor2,refam2,found,fitted,nobs,chi2,quality
   if not found or quality ne 'good' then begin
      print,self,'Error getting transformation'
      return
   endif
