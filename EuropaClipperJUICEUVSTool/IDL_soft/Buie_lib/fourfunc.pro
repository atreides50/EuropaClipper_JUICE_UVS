...
pro fourfunc,x,c,f,pder
   ...
   ;Evaluate
   f = const
   for i=0,alen-1 do f = f + a[i]*cos(2.0d0*!dpi*(i+1)*x)
   for i=0,blen-1 do f = f + b[i]*sin(2.0d0*!dpi*(i+1)*x)

   ;Need partial?
   if n_params(0) eq 4 then begin
      pder=dblarr(n_elements(x),nterms)
      pder[*,0] = 1.0
      for i=0,alen-1 do pder[*,2*i+1] = cos(2.0d0*!dpi*(i+1)*x)
      for i=0,blen-1 do pder[*,2*i+2] = sin(2.0d0*!dpi*(i+1)*x)
   endif
end
