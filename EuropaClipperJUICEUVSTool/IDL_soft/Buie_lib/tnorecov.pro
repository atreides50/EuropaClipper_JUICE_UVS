         ...
         case event.value of
            ...
            'Show Object Info' : begin
               if (*state).select ge 0 then begin
                  ...
                  if (*state).db ne '' then begin
                     openmysql,dblun,(*state).db
                     cmd='select orbtype,quality,hv from dclass,pos'+ $
                         ' where dclass.objectid='+ $
                         quote((*(*state).id)[(*state).select])+ $
                         ' and dclass.objectid=pos.objectid;'
                     mysqlquery,dblun,cmd,orbtype,quality,hvmag, $
                        format='a,i,f',ngood=count
                     if count ne 0 then begin
                        print,'Dynamical class is ',orbtype,', Q=',strn(quality),', Hv=',hvmag
                     endif
                     free_lun,dblun
                  endif
               endif
...
pro tnorecov,OBSFILE=obsfile,OBSCODE=obscode,AMCRIT=amcrit,DT=dt, $
   ...
   if badpar(db,[0,7],0,caller='TNORECOV: (DB) ',default='') then return
   ...
   state = ptr_new({ $
      ...
      db: db, $                  ; Name of database for extra information
