...
pro astsolve,x,y,in_xi,in_eta,terms,renormfac,bad,cxi,ceta, $
   ...
   repeat begin
      ...
      progressinfo=[progressinfo, $
         string('xi:  chisq=',chisq/float(countgood-1),', scatter=',xiscat, $
            ', worst resids',minmax(xi[zg]-xfit),countgood,pass, $ $
            format='(a,f6.2,a,f4.2,a,2(1x,f5.2),1x,i6," stars, pass ",i2)')]
      if not silent then print,progressinfo[-1]
      ...
      progressinfo=[progressinfo, $
         string('eta: chisq=',chisq/float(countgood-1),', scatter=',etascat, $
