...
;print,'A psfcount',psfcount,', topclip',topclip
...
;print,'B psfcount',psfcount,', topclip',topclip
   avgclip,psfstack,psf,/silent,ntopclip=fix(psfcount*topclip)

   ; Compute the actual center (peak) of the PSF generated
   basphote,1.0,psf,1.0,dw+1,dw+1,dw/2.0,0.,-1., $
...
;zf=4
;setwin,0,xsize=sz*2*zf,ysize=sz*zf
;psf_big=rebin(psf,zf*sz,zf*sz,/sample)
;tv,bytscl(psf_big,min=0,max=0.1,top=255),0
      ; this builds a radial profile of the PSF
      xv   = findgen(sz)
      ...
      r=r[idx]
      f=f[idx]
      ; generate a smoothed version of the radial profile
      lowess,r,f,3,fsmo,order=2
      z=where(fsmo eq min(fsmo))
      if not silent then print,'min at ',r[z[0]],' corner at ',max(r)
      rtail=[r[z[0]]<dw,dw]
      ; only do this if the minimum is not at the end
      if rtail[0] ne rtail[1] then begin
         tcorr=[1.0,0.0]
         coeff=poly_fit(rtail,tcorr,1)
         z=where(rarr ge rtail[0])
         ;psf[z]=psf[z]*poly(rarr[z],coeff)
         psf[z]=min(fsmo)*poly(rarr[z],coeff)>0
      endif
;psf_big=rebin(psf,zf*sz,zf*sz,/sample)
;tv,bytscl(psf_big,min=0,max=0.1,top=255),1
;setwin,1
;plot,r,f,/nodata
;oplot,r,f,psym=8,color='0000ff'xl
;oplot,r,fsmo
;;asdf
   endif

   psf = psf/total(psf)
;print,total(psf),max(psf)
